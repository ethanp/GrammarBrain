...
from GrammarBrain.brown_data.util.brown_pos_map import pos_vector_mapping
...
def generalization_error(net, length, numb, punct):
    sentence_tuples = get_nice_sentences_as_tuples(MIN=length, MAX=length,
                                                   include_numbers=numb,
                                                   include_punctuation=punct)

    exper_data = SequenceClassificationDataSet(inp=len(pos_vector_mapping), target=2)
...
def first_vsn():
    ...
    with open(PATH+OUT_CSV, 'wb') as output:
        ...
        for sentence_len in range(20, 30):
            ...
            gen_error = generalization_error(loaded_network, sentence_len, True, True)
...
def second_version():
    ...
    with open(PATH+'ext_2.csv', 'wb') as output:
        ...
        for sentence_len in range(20, 30):
            print 'error on len:', sentence_len
            gen_error = generalization_error(loaded_network, sentence_len, False, False)
            error_at_len = ['Generalization Error', sentence_len, gen_error]
            writer.writerow(error_at_len)
            print error_at_len
